...
void PlayerCreationManager::loadProfessionDefaultsInfo() {
    ...
    for (int i = 0; i < dtiff.getTotalRows(); ++i) {
        ...
        for (int j = 1; j < 10; ++j) {
            int value = 0;
            row->getValue(j, value);
            pdi->setAttributeMod(j - 1, value);
        }
    }
...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
    ...
    ManagedReference<CreatureObject*> playerCreature = zoneServer->createObject(serverObjectCRC, 2).castTo<CreatureObject*>();
